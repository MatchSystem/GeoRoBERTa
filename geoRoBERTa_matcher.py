...
from tag_model.tagging import get_tags, get_tags2
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a,text_b, TokenList, pattern,TokenList2, pattern2,label=None):
...
class Processor(DataProcessor):
  ...
  def get_labels(self):
    return ["NoMatch", "PartialMatch", "Match"]
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      if set_type == "test":   
          ...
          TokenList2 = line[-3]
          pattern2 = line[-2]
      examples.append(InputExample(guid=guid, text_a=text_a,text_b=text_b, TokenList=TokenList, pattern=pattern, TokenList2=TokenList2, pattern2=pattern2, label=label ))
    return examples
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.text_b:
            tag_sequence = get_tags2(example)
        ...
        features.append(InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids,token_tag_sequence_a = token_tag_sequence_a, token_tag_sequence_b = token_tag_sequence_b,len_seq_a = len_seq_a, len_seq_b = len_seq_b, input_tag_ids = None,input_tag_verbs = None,input_tag_len = None,orig_to_token_split_idx=orig_to_token_split_idx,label_id=label_id))
...
def convert_examples_to_featuresTEST(examples, label_list, max_seq_length, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    features = []
    for (ex_index, example) in enumerate(examples):
        tokens_a = []
        ...
        features.append(InputFeaturesTEST(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids,token_tag_sequence_a = token_tag_sequence_a, token_tag_sequence_b = token_tag_sequence_b,len_seq_a = len_seq_a, len_seq_b = len_seq_b, input_tag_ids = None,input_tag_verbs = None,input_tag_len = None,orig_to_token_split_idx=orig_to_token_split_idx))
    ...
    for example in features:
        ...
        if example.token_tag_sequence_b != None:
            ...
            for idx, doc_tag_ids in enumerate(tag_ids_list_b):
                ...
                doc_input_tag_ids = doc_tag_ids + [2] #SEP
...
def main():
    ...
    parser.add_argument("--Roberta_model", default="bert-base-uncased", type=str,
                        help="Bert pre-trained model selected in the list: bert-base-uncased, "
                        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
                        "bert-base-multilingual-cased, bert-base-chinese, roberta.")
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(args.Roberta_model, max_len=512)
    train_examples = None
    ...
    tag_config = TagConfig(tag_vocab_size=vocab_size, hidden_size=10,layer_num=1,output_dim=10,dropout_prob=0.1)
    model = BertForSequenceClassificationTag.from_pretrained(args.Roberta_model, num_labels = num_labels,tag_config=tag_config)
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                predict_model = BertForSequenceClassificationTag.from_pretrained(args.Roberta_model, state_dict=model_state_dict,num_labels = num_labels,tag_config=tag_config)
            ...
            if eval_loss > the_last_loss:
              ...
              #  print('trigger times:', trigger_times)
                ...
                trigger_times = 0
            the_last_loss = eval_loss

    if args.do_eval:
        npp = args.num_train_epochs - best_epoch
        for epoch in trange(int(args.num_train_epochs) - int(npp), desc="Epoch"):
        #for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            eval_examples = processor.get_dev_examples(args.data_dir)
            eval_features = convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer)
            eval_features = transform_tag_features(eval_features, tag_tokenizer, args.max_seq_length)
            ...
            output_model_file = os.path.join(args.output_dir, str(epoch) + "_pytorch_model.bin")
            model_state_dict = torch.load(output_model_file)
            predict_model = BertForSequenceClassificationTag.from_pretrained(args.Roberta_model,
    ...
    if args.do_predict:
        ...
        eval_features = convert_examples_to_featuresTEST(
